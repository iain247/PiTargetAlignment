...
def delete_waypoint(vehicle):
    """
    Upload the mission with the first WP in the current mission list deleted
    """
    # Get the set of commands from the vehicle
    cmds = vehicle.commands
    cmds.download()
...
def distance_to_current_waypoint(vehicle):
    ...
    if currentwaypoint==0:
        return None
    missionitem=vehicle.commands[currentwaypoint-1] #commands are zero indexed
    lat = missionitem.x
    lon = missionitem.y
    alt = missionitem.z
...
def bearing_to_next_waypoint(vehicle):
...
while True:
    ...
    if mode == 'GROUND':
        ...
        initiate = input("Take-Off? y/n: ")
        if initiate == "y":
            print ("Time for take-off!!")
            mode = 'TAKEOFF'
            ...
            for c in cnts:
                ...
                if len(approx) >= 4 and len(approx) <= 6:
                    ...
                    if keepDims and keepSolidity and keepAspectRatio:
                        ...
                        PID_x = PID(10,1,1).start()
                        PID_y = PID(10,1,1).start()
                        
                        #continuously update velocities
                        while Dx > 1 and Dy > 1:
                            v_x = PID_x.update(Dx)
                            v_y = PID_x.update(Dy)
                            
                            #limit the max speeds
                            v_x = saturate(Dx, -max_y_speed, max_y_speed)
                            v_y = saturate(Dy, -max_x_speed, max_x_speed)
                            
                            #set drone velocities
                            set_velocity_body(vehicle, v_y/2, v_x/2, 0.0)
